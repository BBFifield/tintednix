...
in {
      ...
      type = lib.types.attrsOf (lib.types.submodule {
        options = {
          ...
          path = lib.mkOption {
            ...
            default = "";
            example = "./mozilla";
            description = "Path where all the color scheme files should be stored starting from the /etc directory.";
          };
  ...
  config = lib.mkIf (cfg.enable) (
    let
      ...
      #       case "$cmd" in
      #         get)
      #           grep "$scheme=" "$directory/tintednix/settings.txt" | cut -d '=' -f 2
      #     '')
      #   (lib.mkOrder 2000
      #     ''
      #           ;;
      #         *)
      #           echo "Unknown command '$cmd'" >&2; exit 3
      #           ;;
      #       esac
      #     '')
      # ];
      # targetHooks' = scriptParts;
      package = {
        name = "base16-gtk";
        package = let
          defaultScheme = lib.head (lib.filter (scheme: (scheme.name == config.tintednix.defaultSchemeName)) base16schemes);
    ...
    in
      lib.mkMerge [
        {
          # environment = targetFiles;
        }
        # {
        #   environment.etc."tintednix/settings.txt".text = let
